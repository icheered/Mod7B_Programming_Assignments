...
template <typename NODETYPE> class List
{
  public:
	...
	bool concatenate(List<char>& toAppend) { //ref is used as list has to be emtied aswell and in order to save copy operation
		if (toAppend.isEmpty()) return false;
		while (true) {
			if (toAppend.firstPtr == nullptr) return true;
			ListNode<NODETYPE>* appending = getNewNode((NODETYPE)toAppend.firstPtr->data);
			this->lastPtr->nextPtr = appending;
			this->lastPtr = appending;
				ListNode<NODETYPE>* temp = toAppend.firstPtr;
				toAppend.firstPtr = toAppend.firstPtr->nextPtr;
				delete temp;
		}
		return true;
	}
